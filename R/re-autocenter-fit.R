...
re_autocenter_specs <- function(bframe, unresolved = TRUE) {
  ...
  for (id in ids) {
    ...
    # This branch deliberately retains predictor-local group-effect IDs.
    # Cross-predictor and strict latent S2Z blocks remain outside its support
    # boundary, so the framed coefficient number is the complete dimension
    # map for every eligible automatic-centering block.
    latent <- FALSE
    cross <- FALSE
