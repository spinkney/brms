...
stan_fe <- function(bframe, prior, stanvars, threads, primitive,
  ...
  if (s2z) {
    ...
    inactive_s2z <- setdiff(coef_s2z, active_coef_s2z)
    ...
    if (length(inactive_s2z)) {
      inactive_names <- info_s2z$qnames[inactive_s2z]
...
stan_re <- function(bframe, prior, normalize, ...) {
  ...
  for (bfl in Filter(has_re_s2z, all_bframel(bframe))) {
    ...
    # Ordinal thresholds participate in the same omitted-mean system as every
    # local S2Z block. Route even a singleton ordinal block through the dense
    # joint kernel so the repeated threshold rows and affine slope map have one
    # reference implementation instead of being duplicated across scalar and
    # independent fast paths.
  ...
  if (any(grepl(
    "(mdivide_left_tri_low|mdivide_right_tri_low|cholesky_decompose|chol2inv)_brms\\(",
...
.stan_re <- function(id, bframe, prior, threads, normalize,
  ...
  s2z_ordinal <- isTRUE(r$s2z[1]) && !is.null(joint_s2z) &&
    isTRUE(joint_s2z$infos[[1L]]$ordinal)
  if (s2z_ordinal && center_fisher) {
    stop2("The automatic-centering precursor is not yet supported for ",
          "ordinal S2Z effects.")
  }
  fisher_info <- NULL
  if (center_fisher) {
    fisher_info <- stan_re_s2z_fisher_info(
  ...
  if (is_cor) {
    ...
    if (identical(mode, "centered")) {
      ...
      str_add(out$tpar_comp) <- glue(
        ...
        "      }}\n",
        "      white_center_re = mdivide_left_tri_low_brms(\n",
...
# The response-free Fisher-information catalog and chart core are kept
# together so both ordinary and S2Z group effects use the same reference.
...
stan_re_s2z_fisher_gq_comp <- function(id, r, fisher_info, L = NULL,
  ...
  proposal <- stan_re_s2z_fisher_comp(
    ...
    diag_scale = diag_scale, rho = rho, mean_rho = mean_rho
  )
  glue(
...
# unweighted within-level Gram matrices once so generated-quantity proposals
# only combine the expected observation precision and covariance parameters.
...
stan_re_s2z_fisher_comp <- function(id, r, fisher_info, L = NULL,
  ...
  # reliability I * tau^2 / (1 + I * tau^2). Fixed ordinary designs use the
  # transformed-data exposure; other likelihood weights remain draw-specific.
  if (M == 1L) {
  ...
  if (constrained_s2z) {
    ...
    return(glue(
      ...
      "cholesky_decompose_brms(sum_white_post_cov_fisher_s2z);\n",
      "    for (j in 1:N_{id}) {{\n",
  ...
  reliability_comp <- if (diagonal) {
    glue(
      ...
      "        unit_column_fisher_s2z = mdivide_left_tri_low_brms(\n",
...
stan_re_s2z_partial_cor_transform <- function(id) {
  glue(
    ...
    "mdivide_left_tri_low_brms(L_partial_s2z, r_s2z_{id}[j]'))';\n",
...
.stan_re_s2z_joint_block <- function(id, set, bframe, prior, threads,
  ...
  # This map is a deterministic function of formula structure and the checked
  # affine design identity, so construct it outside the autodiff graph.
  str_add_list(out) <- stan_re_s2z_H_code(info)
  ...
  if (s2z_partial && is_cor) {
    ...
    for (j in seq_len(M)) {
      str_add(out$tpar_comp) <- stan_re_s2z_partial_independent_transform(
        id, j, glue("r_s2z_{id}[, {j}]"), glue("sd_{id}[{j}]")
      )
    }
  } else {
    str_add(out$tpar_comp) <- glue(
  ...
  if (use_matheron && !s2z_center && !s2z_partial) {
    # The non-centered S2Z basis is already white in coefficient space.
    str_add(out$tpar_comp) <- glue(
...
.stan_re_s2z_joint_matheron <- function(set, prior, threads, normalize, ...) {
  ...
  for (b in seq_along(infos)) {
    ...
    id <- infos[[b]]$id
    s2z_mode <- re_s2z_center_mode(r)
    if (identical(s2z_mode, "centered")) {
      str_add(out$tpar_prior) <- glue(
        "    - (N_{id} - 1) * ",
        "sum(log(diagonal(L_Sigma_s2z_{id})))\n"
      )
    } else if (s2z_mode %in% c("partial", "auto")) {
      str_add(out$tpar_prior) <- glue(
        "    + log_det_partial_s2z_{id}\n"
      )
    }
...
# Legacy Plan 02 generator for the physically centered endpoint. Keeping this
# path byte-identical preserves compiled-code and posterior compatibility.
# The omitted common
...
.stan_re_s2z_centered_legacy <- function(id, bframe, prior, threads, normalize,
                                          out = list()) {
  lpdf <- stan_lpdf_name(normalize)
...
.stan_re_s2z <- function(id, bframe, prior, threads, normalize, out = list(),
                         fisher_info = NULL) {
  r_center <- subset2(bframe$frame$re, id = id)
  if (identical(re_s2z_center_mode(r_center), "centered")) {
    return(.stan_re_s2z_centered_legacy(
      id, bframe = bframe, prior = prior, threads = threads,
      normalize = normalize, out = out
    ))
  }
  ...
  str_add(out$gen_comp) <- glue(
    ...
    "    vector[M_{id}] forward_solve_s2z = mdivide_left_tri_low_brms(",
    "L_P_s2z_{id}, h_s2z);\n",
    "    vector[M_{id}] r_mean_s2z = (mdivide_right_tri_low_brms(",
    "forward_solve_s2z', L_P_s2z_{id}))';\n",
    "    vector[M_{id}] z_mean_s2z;\n",
    "    for (k in 1:M_{id}) z_mean_s2z[k] = std_normal_rng();\n",
